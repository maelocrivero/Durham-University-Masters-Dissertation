...
def stream_dataframe(filepath, chunk_size, use_cols=None):
    """
    Streams a DataFrame from a CSV file in chunks, optionally selecting only certain columns.
    Raises errors if the file path is invalid or the file is not a CSV.
    """
...
def binning_from_chunks(filepath, chunk_size, continuous_columns=None, num_bins=4, sample_size=10000, use_cols=None):
    """
    Returns bin edges for continuous variables by processing the dataset in chunks.
    Assumes the continuous columns are specified or automatically detected.

    Parameters:
        filepath (str): Path to CSV file.
        chunk_size (int): Number of rows per chunk.
        continuous_columns (list of str, optional): If None, auto-detects from first chunk.
        num_bins (int): Number of bins to use for discretization.
        sample_size (int): Number of rows to use for estimating bin boundaries (reservoir sampling).
        use_cols (list of str, optional): Subset of columns to load from the CSV.
    """
    sample = []
    total_seen = 0
...
def bin_continuous_columns(pspace, columns, num_bins=4, strategy='quantile', return_edges=False):
    """
    Bins specified continuous columns in a DataFrame.

    Parameters:
        pspace: ProbabilitySpace object containing the DataFrame as pspace.df.
        columns (list): List of column names to bin.
        num_bins (int): Number of bins to create.
        strategy (str): 'quantile' for equal-frequency, 'uniform' for equal-width bins.
        return_edges (bool): If True, also returns bin edges used.

    Returns:
        pd.DataFrame: Modified DataFrame with binned columns.
        (Optional) dict: Bin edges used per column.
    """
...
def apply_bin_boundaries(pspace_or_df, bin_boundaries, handle_missing="error"):
    """
    Apply bin boundaries to continuous variables.

    Parameters
    ----------
    pspace_or_df : ProbabilitySpace or pd.DataFrame
    bin_boundaries : dict
        { column_name: list of bin edges }
    handle_missing : str
        'error' → raise error if column has NaNs
        'ignore' → skip rows with NaNs
        'fill'   → fill NaNs with a special bin
    """
    import pandas as pd

    # Handle input
    if hasattr(pspace_or_df, "df"):  # ProbabilitySpace
