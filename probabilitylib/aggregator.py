...
def combine_means(results):
    """results: list of (mean, chunk_weight) tuples"""
...
def combine_EDF_or_CDF(chunk_results, var_type="discrete", plot=False):
    """
    Combine chunk-level EDFs/CDFs correctly by converting cumulative sums
    to per-value weights, summing across chunks, and computing a global cumulative sum.
    
    chunk_results: list of tuples [(x_values_chunk, cum_weights_chunk), ...]
    var_type: currently supports "discrete"
    plot: whether to plot the combined EDF
    """
    if var_type == "discrete":
        ...
        for _, cdf_vals in chunk_results:
            # Convert cumulative sums → per-bin weights
...
def combine_mutual_information_outputs(outputs):
    """
    Given a list of outputs from accumulate_joint_and_marginals,
    combine their joint counts, marginal counts, and total weights.
    
    Each output is a tuple:
      (joint_counts, marginal_x, marginal_y, total_weight)
    """
    combined_joint = {}
    combined_marginal_x = {}
    combined_marginal_y = {}
...
def combine_mutual_information(outputs):
    """
    Combine partial counts from chunks and compute the final MI.
    """
...
def combine_bayesian_models(results, top_k=10, score_type='hybrid', return_all=False):
    """
    Combine Bayesian models across chunks with direction-aware edge aggregation.

    Args:
        results: list of (model, bic_scores) tuples from chunks.
        top_k: how many edges to retain (or consider for the DAG).
        score_type: 'softmax', 'frequency', or 'hybrid'.
        return_all: if True, return full edge stats.

    Returns:
        final_model_edges: list of edges in the DAG (u,v)
        sorted_edges: optional, ranked list of edges with final scores
        full_stats: optional, dict of edge → (freq, softmax_score, hybrid_score)
    """
    from collections import defaultdict
    import numpy as np
    import networkx as nx

    # Accumulate stats for directed edges
...
def aggregate_and_build_network(
    ...
    """
    Aggregates sampled chunks and performs bootstrapped Bayesian structure learning.
    Returns consensus edges (those appearing in ≥ consensus_threshold × n_iterations).
    """
    ...
    # Step 2: Perform structure learning multiple times with bootstrapped samples
    edge_counts = Counter()
    ...
    # Step 3: Filter edges by consensus threshold
