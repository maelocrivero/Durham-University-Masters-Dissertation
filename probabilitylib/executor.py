...
def get_total_weights(filepath, chunk_size=10000, use_cols=None):
    """
    Computes the total weight across all chunks.
    If no weights column exists, returns the total number of rows.
    """
    total_weights = 0
...
def _apply_func(chunk, func, *args, **kwargs):
    """
    Helper to call the correct logic for your functions based on func.__name__.
    Extracted from your original logic to avoid duplication.
    """
    if func.__name__ == "expected_mean":
        ...
        if con == "dependent":
            ...
            # Return partial sums for later aggregation
            return pA, pB
        ...
        # Drop rows with NaNs in the variable column
